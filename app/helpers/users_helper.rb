module UsersHelper
  def get_status(user)
    if current_user.action_type != 'wait' || current_user.action_type != 'no_Game'
      if user.is_dead
        status = '死亡'
      ...
      if current_user.action_type == 'night' && !current_user.is_dead
        if current_user.job_id == 1
          if user.job_id == 1
            ...
            link_to '弱', night_village_games_path(village_id: current_user.village_id, user_id: user.id, level: 1), remote: true
          end
